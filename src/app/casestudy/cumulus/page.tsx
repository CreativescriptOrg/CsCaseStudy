...
import data from "@/json/cumulus.json";
...
export default function Cumulus() {
	...
	return (
		<div style={{ "--theme-color": data.theme_color } as React.CSSProperties}>
			...
			<div className='container cumulus_mockup_grid mockup_grid'>
			...
			<div className='container mockup_grid'>
				{data.mockups_4.map((el: any, i: number) => (
					<Media key={i} data={el} />
				))}
			</div>

			<ProjectDetailsSection data={data.projectDetails_5} />
		</div>
	);
}
